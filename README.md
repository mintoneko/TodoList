...
仓库包含适用于 Nginx 的配置文件：[deploy/nginx-vue3-todo-list.conf](deploy/nginx-vue3-todo-list.conf)。站点部署到 `/var/www/vue3-todo-list`，使用 `todo.loliowo.com`，HTTP 会自动跳转到 HTTPS。
...
安装脚本会创建每日续签任务；本部署使用 Let’s Encrypt 作为 ACME CA。
...
首次申请时先启用临时 HTTP 配置，让 acme.sh 通过网站根目录完成 HTTP-01 校验：
...
ssh <server> "sudo /root/.acme.sh/acme.sh --issue --server letsencrypt -d todo.loliowo.com -w /var/www/vue3-todo-list"
ssh <server> "sudo install -d -m 755 /etc/nginx/ssl/todo.loliowo.com && sudo /root/.acme.sh/acme.sh --install-cert -d todo.loliowo.com --key-file /etc/nginx/ssl/todo.loliowo.com/privkey.pem --fullchain-file /etc/nginx/ssl/todo.loliowo.com/fullchain.pem --reloadcmd 'systemctl reload nginx'"
```
